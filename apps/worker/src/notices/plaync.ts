...
function buildArticleUrl(urlPattern: unknown, articleId: string) {
  if (typeof urlPattern === "string" && urlPattern.includes("{articleId}")) {
    return urlPattern.replace("{articleId}", articleId);
  }
  // Conservative fallback (pattern appears in list responses, but keep this just in case).
  return `https://aion2.plaync.com/ko-kr/board/notice/view?articleId=${encodeURIComponent(articleId)}`;
}

function toMeta(raw: unknown): PlayncArticleMeta | null {
  ...
  const meta: PlayncArticleMeta = {
    id,
    title,
    url: buildArticleUrl(urlPattern, id)
  };
...
async function fetchJson(url: string) {
  const res = await fetch(url, {
    headers: {
      ...
      "user-agent": "aion2-hub/0.1 (notices-sync)"
    }
  });
...
export async function fetchPinnedArticleMetas(alias: PlayncBoardAlias) {
  ...
  for (const item of list) {
    ...
    const meta = toMeta((item as Record<string, unknown>).articleMeta);
...
export async function fetchMoreArticleMetas(
  ...
  const metas = list.map(toMeta).filter((m): m is PlayncArticleMeta => m !== null);
...
export async function fetchArticleDetail(alias: PlayncBoardAlias, articleId: string) {
  ...
  const meta = toMeta(data.article?.contentMeta);
